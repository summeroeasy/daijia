...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private ProfitsharingRuleFeignClient profileRulesFeignClient;
    ...
    public Boolean endDrive(OrderFeeForm orderFeeForm) {
        //1 根据orderId 获取订单信息，判断当前订单是否司机接单
        ...
        Long orderNumber = orderInfoFeignClient.getOrderNumByTime(startTime, endTime).getData();

        //远程调用
        RewardRuleRequestForm rewardRuleRequestForm = new RewardRuleRequestForm();
        rewardRuleRequestForm.setStartTime(orderInfo.getStartServiceTime());
        rewardRuleRequestForm.setOrderNum(orderNumber);
        RewardRuleResponseVo rewardRuleResponseVo = rewardRuleFeignClient.calculateOrderRewardFee(rewardRuleRequestForm).getData();

        //5 计算分账信息
        ProfitsharingRuleRequestForm profitsharingRuleRequestForm = new ProfitsharingRuleRequestForm();
        profitsharingRuleRequestForm.setOrderAmount(feeRuleResponseVo.getTotalAmount());
        profitsharingRuleRequestForm.setOrderNum(orderNumber);
        ProfitsharingRuleResponseVo profitsharingRuleResponseVo = profileRulesFeignClient
    ...
    public Boolean endDriveThread(OrderFeeForm orderFeeForm) {
        ...
        //4 计算系统奖励
        CompletableFuture<Long> orderNumberCompletableFuture = CompletableFuture.supplyAsync(() -> {
            String startTime = new DateTime(orderInfo.getStartServiceTime()).toString("yyyy-MM-dd") + " 00:00:00";
            String endTime = new DateTime(orderInfo.getStartServiceTime()).toString("yyyy-MM-dd") + " 23:59:59";
            Long orderNumber = orderInfoFeignClient.getOrderNumByTime(startTime, endTime).getData();
            return orderNumber;
        ...
        CompletableFuture<RewardRuleResponseVo> rewardRuleResponseVoCompletableFuture = orderNumberCompletableFuture
                .thenApplyAsync((orderNumber) -> {
            //远程调用
            //封装参数
            RewardRuleRequestForm rewardRuleRequestForm = new RewardRuleRequestForm();
            rewardRuleRequestForm.setStartTime(orderInfo.getStartServiceTime());
        ...
        CompletableFuture<ProfitsharingRuleResponseVo> profitsharingRuleResponseVoCompletableFuture = feeRuleResponseVoCompletableFuture
                .thenCombineAsync(orderNumberCompletableFuture,
                (feeRuleResponseVo, orderNumber) -> {
                    ProfitsharingRuleRequestForm profitsharingRuleRequestForm = new ProfitsharingRuleRequestForm();
                    profitsharingRuleRequestForm.setOrderAmount(feeRuleResponseVo.getTotalAmount());
                    profitsharingRuleRequestForm.setOrderNum(orderNumber);
                    ProfitsharingRuleResponseVo profitsharingRuleResponseVo = profileRulesFeignClient
    ...
    /**
     * 根据司机ID查询订单分页信息
     * 此方法用于根据提供的司机ID，查询并返回该司机的订单信息的分页视图
     * 主要用于界面展示，帮助了解司机的订单情况
     *
     * @param driverId 司机ID，用于指定查询哪个司机的订单
     * @param page     页码，用于分页查询，指定要查询的页数
     * @param limit    每页显示数量，用于分页查询，指定每页要显示的订单数量
     * @return 返回一个分页视图对象PageVo，包含查询到的订单信息和分页数据
     */
    @Override
